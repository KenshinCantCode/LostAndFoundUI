...
<body class="bg-paper text-ink antialiased" data-site-url="<?= SITE_URL ?>">
    <nav class="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur-md">
        <div class="mx-auto flex max-w-7xl items-center justify-between gap-3 px-5 py-4 lg:px-8">
            <a class="font-display flex items-center gap-3 text-lg font-bold tracking-tight text-ink" href="<?= SITE_URL ?>">
                ...
                <span class="truncate text-xl sm:text-2xl"><?= SITE_NAME ?></span>
            </a>
            ...
            <div class="hidden items-center gap-8 lg:flex" data-nav-menu>
                ...
                <div class="flex items-center gap-3">
                    <?php if ($auth->isLoggedIn()): ?>
                        ...
                        <div class="relative">
                            <button class="flex items-center gap-2 rounded-full border border-slate-200 bg-white py-1.5 pl-1.5 pr-3 text-sm font-semibold text-slate-700 shadow-sm" type="button" data-menu-toggle="user-menu" aria-controls="user-menu" aria-expanded="false"><span class="flex h-8 w-8 items-center justify-center rounded-full bg-mint text-sm text-white"><i class="fas fa-user"></i></span><span class="max-w-[10rem] truncate"><?= sanitize($_SESSION['full_name']) ?></span><i class="fas fa-chevron-down text-[10px] text-slate-400"></i></button>
            ...
            <div class="absolute left-0 right-0 top-full hidden border-b border-slate-200 bg-white p-5 shadow-lg lg:hidden" data-nav-mobile>
                <div class="space-y-2">
                    <a class="block py-2 font-semibold text-slate-700" href="<?= SITE_URL ?>">Home</a>
                    <a class="block py-2 font-semibold text-slate-700" href="<?= SITE_URL ?>/search.php">Search</a>
                    <?php if ($auth->isLoggedIn()): ?>
                        <a class="block py-2 font-semibold text-slate-700" href="<?= SITE_URL ?>/dashboard.php">Dashboard</a>
                        <a class="block py-2 font-semibold text-slate-700" href="<?= SITE_URL ?>/my-reports.php">My Reports</a>
                        <a class="block py-2 font-semibold text-slate-700" href="<?= SITE_URL ?>/my-claims.php">My Claims</a>
                        <a class="block py-2 font-semibold text-slate-700" href="<?= SITE_URL ?>/profile.php">Profile</a>
                        <a class="block py-2 font-semibold text-slate-700" href="<?= SITE_URL ?>/report-lost.php">Report Lost</a>
                        <a class="mt-2 block rounded-lg bg-ink px-4 py-2 text-center font-bold text-white" href="<?= SITE_URL ?>/logout.php">Logout</a>
                    <?php else: ?>
                        <a class="mt-2 block rounded-lg bg-ink px-4 py-2 text-center font-bold text-white" href="<?= SITE_URL ?>/register.php">Register</a>
                    <?php endif; ?>
                </div>
            </div>
        </div>
